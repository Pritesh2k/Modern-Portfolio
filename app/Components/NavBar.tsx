...
export const Navbar: React.FC = () => {
    ...
    return (
        <div className="fixed bottom-12 left-1/2 transform -translate-x-1/2 z-50">
            ...
            {/* Morphing GlassSurface: Circle -> Expanded Navbar */}
            <AnimatePresence>
                <motion.div
                    key="glass-toggle"
                    onClick={() => setIsOpen(!isOpen)}
                    className="absolute bottom-0 left-1/2 -translate-x-1/2 z-50 cursor-pointer flex items-center justify-center"
                    animate={{
                        width: isOpen ? "30vw" : 64, // 4rem circle when closed
                        maxWidth: isOpen ? 480 : 64,
                        height: isOpen ? 80 : 64,
                        borderRadius: isOpen ? 24 : 100, // fully round when closed
                    }}
                ...
                >
                    <GlassSurface
                        ...
                        backgroundOpacity={0}
                        brightness={1}
                        className="relative flex items-center justify-center px-6 pointer-events-auto bg-transparent"
                    >
                        {isOpen ? (
                            <>
                                {/* Navbar Items */}
                                <motion.ul
                                    className="flex gap-4 sm:gap-6 items-center w-full justify-center"
                                ...
                                {/* Close Icon */}
                                <X size={20} className="text-white absolute right-0 top-1/2 -translate-y-1/2 z-10" />
                            </>
                        ) : (
            ...
            <AnimatePresence>
                {hoveredIdx !== null && isOpen && (
                    <motion.div
                        ...
                        className="absolute flex items-center justify-center bottom-5 left-1/2 -translate-x-1/2  w-100 h-56"
                    >
                        <span className="scale-y-200 flex justify-center items-center w-full h-full text-center font-[Roboto_Flex] font-light text-white tracking-widest drop-shadow-[0_0_10px_var(--color-white)] text-[10vh] leading-none">
